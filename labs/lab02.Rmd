...
```{r}
ggplot(polls)+
  geom_point(aes(x = end_date, y = approve_adjusted), color = "palegreen3", alpha = 0.5) +
  geom_smooth(aes(x = end_date, y = approve_adjusted), color = "springgreen4") +
  geom_point(aes(x = end_date, y = disapprove_adjusted), color = "plum3", alpha = 0.5) + 
...
```{r}
ggplot(rent %>% filter(room_in_apt	== 0), aes(x = price, fill = as.factor(beds))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~ year)
```
