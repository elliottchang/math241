...
```{r}
weekends <- accidents %>% filter(day_of_week == "Saturday" | day_of_week == "Sunday")
weekdays <- accidents %>% filter(day_of_week != "Saturday" & day_of_week != "Sunday")
ggplot(data = weekdays %>% group_by(severity), )
```
